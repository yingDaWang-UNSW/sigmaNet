...
realLR_dataset_test = realLR_dataset_test.map(loadRealLR, num_parallel_calls=tf.data.experimental.AUTOTUNE).shuffle(args.batch_size*5).batch(1).prefetch(args.batch_size*5)
realHR_and_synLR_dataset_test = tf.data.Dataset.list_files(args.dataset_dir+'testB/*.png')
realHR_and_synLR_dataset_test = realHR_and_synLR_dataset_test.map(loadRealHRandBC, num_parallel_calls=tf.data.experimental.AUTOTUNE).shuffle(args.batch_size*5).batch(1).prefetch(args.batch_size*5)
...
def DiscriminatorSRGAN(args):
    ...
    x = tf.keras.layers.LeakyReLU(alpha=0.2)(x)
    xOut = tf.keras.layers.Dense(1)(x)
    '''
...
with strategy.scope():
    ...
    def meanAbsoluteError(labels, predictions):
        per_example_loss = tf.reduce_mean(tf.abs(labels-predictions), axis = list(np.linspace(1,args.nDims,args.nDims,dtype='uint8')))
    ...
    def meanSquaredError(labels, predictions):
        per_example_MSE = tf.reduce_mean(((labels-predictions))**2, axis = list(np.linspace(1,args.nDims,args.nDims,dtype='uint8')))
        return tf.nn.compute_average_loss(per_example_MSE, global_batch_size=args.batch_size)
       
    def sigmoidCrossEntropy(labels, logits):
        per_example_sxe = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=labels), axis = list(np.linspace(1,args.nDims,args.nDims,dtype='uint8')))
    ...
    def lsganLoss(disc_real_output, disc_generated_output):
        real_loss = meanAbsoluteError(tf.ones_like(disc_real_output), disc_real_output)
        generated_loss = meanAbsoluteError(tf.zeros_like(disc_generated_output), disc_generated_output)
    ...
    def advLsganLoss(disc_generated_output):
        adversarial_loss = meanAbsoluteError(tf.ones_like(disc_generated_output), disc_generated_output)
    ...
    def train_step(realLRBatch, HRandBCBatch):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if args.ganFlag:
                ...
                advsrLoss = tf.convert_to_tensor(0.0)
                dsrLoss = tf.convert_to_tensor(0.0)
        ...
        # calculate gradients
        gradGAB = tape.gradient(totalGABLoss, generatorAB.trainable_variables)
        gradGBA = tape.gradient(totalGBALoss, generatorBA.trainable_variables)
        gradDA = tape.gradient(totalDALoss, discriminatorA.trainable_variables)
        gradDB = tape.gradient(totalDBLoss, discriminatorB.trainable_variables)
        gradGsr = tape.gradient(totalGsrLoss, generatorSR.trainable_variables)
        if args.ganFlag:
            gradDsr = tape.gradient(totalDsrLoss, discriminatorSR.trainable_variables)
        # apply gradients
        ...
        return totalGABLoss, totalGBALoss, totalDALoss, totalDBLoss, totalGsrLoss, totalDsrLoss
    ...
    def distributed_train_step(realLRBatch, HRandBCBatch):
        PRGABL, PRGBAL, PRDAL, PRDBL, PRGSRL, PRDSRL = strategy.run(train_step, args=(realLRBatch, HRandBCBatch,))
        return strategy.reduce(tf.distribute.ReduceOp.SUM, PRGABL, axis=None), strategy.reduce(tf.distribute.ReduceOp.SUM, PRGBAL, axis=None), strategy.reduce(tf.distribute.ReduceOp.SUM, PRDAL, axis=None), strategy.reduce(tf.distribute.ReduceOp.SUM, PRDBL, axis=None), strategy.reduce(tf.distribute.ReduceOp.SUM, PRGSRL, axis=None), strategy.reduce(tf.distribute.ReduceOp.SUM, PRDSRL, axis=None)
    ...
    if args.phase == 'train':
        ...
        train_template = ("\rEpoch %4d, Iter %4d, Time %4.4f, Speed %4.4f its/s, GABL: %4.4f, GBAL: %4.4f, DAL: %4.4f, DBL: %4.4f, GSRL: %4.4f, DSRL: %4.4f")
...
#        realHR_and_synLR_dataset_test  
        ...
        for epoch in range(EPOCHS):
            ...
            for x, y in zip(realLR_dataset_dist, realHR_and_synLR_dataset_dist):
                GABL, GBAL, DAL, DBL, GSRL, DSRL = distributed_train_step(x,y)
                totGABL += GABL
                totGBAL += GBAL
                totDAL += DAL
                totDBL += DBL
                ...
                stdout.write(train_template % (epoch+1, num_batches, currentTime-start_time, 1/(currentTime-lastTime),GABL, GBAL, DAL, DBL, GSRL, DSRL))
                stdout.flush()
            ...
            totGSRL /= num_batches
            totDSRL /= num_batches
            print('Mean Epoch Performance: GABL: %4.4f, GBAL: %4.4f, DAL: %4.4f, DBL: %4.4f, GSRL: %4.4f, DSRL: %4.4f' % (totGABL, totGBAL, totDAL, totDBL, totGSRL, totDSRL))
